...
pub fn initFromEnv(allocator: std.mem.Allocator) void {
    if (env.getVarOwned(allocator, "M80_LOG_LEVEL")) |raw| {
    ...
    const path = env.getVarOwned(allocator, "M80_LOG_FILE") catch return;
    defer allocator.free(path);
    if (path.len == 0) return;
    if (fs.cwd().createFile(path, .{ .truncate = true })) |file| {
        log_file = file;
    } else |_| {}
}
...
pub fn log(lvl: Level, comptime fmt: []const u8, args: anytype) void {
    ...
    const ts_ms = sync.milliTimestamp();
    if (log_file) |file| {
